...
```sh
cp config/example.conf config/router.local.conf
$EDITOR config/router.local.conf

bin/homelab-doctor --config config/router.local.conf config validate
bin/homelab-doctor --config config/router.local.conf doctor dns
bin/homelab-doctor --config config/router.local.conf doctor mihomo
bin/homelab-doctor --config config/router.local.conf doctor openvpn
bin/homelab-doctor --config config/router.local.conf doctor firewall
bin/homelab-doctor --config config/router.local.conf doctor router
```
...
每次诊断输出 `[OK]`、`[WARN]`、`[!]` 和计数摘要。退出状态：`0` 无故障（可有警告）、`1` 远程诊断 `[!]`、`2` 本地配置/用法错误、`3` SSH 传输失败。SSH 失败不会伪装成模块 `[!]`。

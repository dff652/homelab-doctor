...
run_mihomo_missing_tools_case() {
    name='mihomo：缺 netstat/ss 降级为警告'
    bare_bin="${TMP_DIR}/bare-bin-mihomo"
    mkdir -p "$bare_bin"
    # 提供进程与配置检查所需命令，唯独不给 netstat/ss。
    ln -sf "$(command -v sh)" "${bare_bin}/sh"
    ln -sf "$(command -v date)" "${bare_bin}/date"
    ln -sf "$(command -v grep)" "${bare_bin}/grep"
    ln -sf "${ROOT}/tests/fixtures/stub-command.sh" "${bare_bin}/pgrep"

    status=0
    output=$(
        {
            cat "${ROOT}/probes/openwrt/common.sh" \
                "${ROOT}/probes/openwrt/mihomo.sh"
            printf '%s\n' 'probe_begin' 'probe_mihomo' 'probe_summary'
        } | env HD_FIXTURE_PROFILE=ok \
            HD_MIHOMO_RUNTIME_CONFIG="${ROOT}/tests/fixtures/ok/mihomo.yaml" \
    ...
    if [ "$status" -eq 0 ] \
        && printf '%s\n' "$output" | grep -Fq '缺少 netstat/ss' \
        && printf '%s\n' "$output" | grep -Fq 'OpenClash/Mihomo 核心进程存在'; then
        printf '[OK]   %s\n' "$name"
        pass=$((pass + 1))
    else
        printf '[!]    %s（status=%s）\n' "$name" "$status"
